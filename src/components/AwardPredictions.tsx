    ...
    <div className="rounded-lg bg-app-card overflow-hidden">
            ...
            <Tooltip>
              ...
              <TooltipContent side="right" className="bg-app-card border-app-border max-w-xs">
                <p className="text-xs text-muted-foreground">
                  {isRevealed
                    ? '第79届戛纳电影节各奖项正式获奖名单及每股分红。'
                    : '电影节闭幕、奖项颁布后，此处将公布实际获奖影片及对应分红金额。当前为预设分红标准。'}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
      ...
      <div className="divide-y divide-app-border/60">
        {awardsData.map((award, index) => (
          <div
            key={index}
            className="group relative flex items-center justify-between px-4 py-3 transition-colors duration-200 hover:bg-app-hover"
          >
            ...
            <div className="flex flex-col gap-0.5 min-w-0">
              ...
              {isRevealed ? (
                <span className="text-xs text-app-gold truncate">
                  {award.actualWinner || award.predictedWinner}
                </span>
              ) : (
